...
Parameters:
 ```
    ...
    - adminConsole.pullSecrets                      # Secrets to pull from prive Docker registry;
 ```
...
```bash
helm install admin-console-operator --namespace <edp_cicd_project> --set name=admin-console-operator --set global.edpName=<edp_cicd_project> --set global.platform=<platform_type> deploy-templates
```
